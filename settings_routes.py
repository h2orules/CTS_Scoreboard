    ...
    *app_module* is a reference to the ``CTS_Scoreboard`` module so we can
    ...
    def route_settings():
        ...
        if flask.request.method == "POST":
            ...
            if "time_standards_file" in flask.request.files:
                ...
                if file and file.filename and file.filename.endswith(".st2"):
                    ...
                    try:
                        ...
                        settings["time_standards"] = base64.b64encode(
                            pickle.dumps(_app.time_standards)
                        ).decode("ascii")
                        ...
                        settings["std_desc_overrides"] = {
                            k: v for k, v in overrides.items() if k in new_tags
                        }
                        modified = True
                    finally:
                        try:
                ...
                if file and file.filename and file.filename.endswith(".rec"):
                    ...
                    try:
                        ...
                        settings["swim_record_sets"] = base64.b64encode(
                            pickle.dumps(_app.swim_record_sets)
                        ).decode("ascii")
                        modified = True
                    finally:
                        try:
            ...
            AD_DIR = os.path.join(
                os.path.dirname(os.path.abspath(_app.__file__)), "static", "ad"
            )
            ...
            if "ad_images" in flask.request.files:
                ...
                for upload in uploaded:
                    ...
                    if ext not in AD_ALLOWED_EXTS:
                        ad_errors.append(
                            "Rejected %s (unsupported type)" % upload.filename
                        )
                        continue
                    try:
                    ...
                    try:
                        ...
                        out_bytes, out_ext, info = ad_image.process_upload(
                            raw, ext, max_dim
                        )
                    except ad_image.AdImageError:
                        logging.exception(
                            "Rejected ad image upload during validation/processing: %s",
                            upload.filename,
                        )
                        ad_errors.append(
                            "Rejected %s (invalid image)" % upload.filename
                        )
                        continue
                    except Exception:
                        logging.exception(
                            "Failed to process ad image upload: %s", upload.filename
                        )
                    ...
                    try:
                        ...
                        logging.exception(
                            "Failed to save processed ad image: %s", upload.filename
                        )
            ...
            if ad_form_submitted:
                ...
                if removed_idx is not None:
                    ...
                    for idx in range(len(ad_list)):
                        if ("ad_up_%d" % idx) in flask.request.form and idx > 0:
                            swap = (idx, idx - 1)
                            break
                        if ("ad_down_%d" % idx) in flask.request.form and idx < len(
                            ad_list
                        ) - 1:
                            swap = (idx, idx + 1)
                            break
                ...
                try:
                    new_interval = int(
                        flask.request.form.get("ad_rotation_interval", "30")
                    )
            ...
            if "message_pages_form" in flask.request.form:
                ...
                new_interval = int(
                    flask.request.form.get("message_rotation_interval", "30")
                )
                if new_interval < 5 or new_interval > 60 or new_interval % 5 != 0:
                    new_interval = 30
                ...
                if settings.get("message_pages") != new_pages:
                    ...
                    msg_changed = True
                if (
                    settings.get("message_overlay_enabled", False)
                    != new_overlay_enabled
                ):
            ...
            if "footer_form" in flask.request.form:
                ...
                if remove_id:
                    ...
                    if text.strip():
                        ...
                        genders = [
                            v
                            for v in flask.request.form.getlist("footer_genders")
                            if v in allowed_genders
                        ]
                        strokes = [
                            v
                            for v in flask.request.form.getlist("footer_strokes")
                            if v in allowed_strokes
                        ]
                        age_groups = [
                            v
                            for v in flask.request.form.getlist("footer_age_groups")
            ...
            if flask.request.headers.get("X-Autosave") == "1":
                errors = [
                    e
                    for e in (schedule_error, standards_error, records_error, ad_error)
                    if e
                ]
                if errors:
            ...
            ad_images.append(
                {
                    ...
                    "size_human": ad_image.format_size(size_bytes)
        ...
        for tag_key, name_key in [
            ("team_home_tag", "team_home"),
            ("team_guest1_tag", "team_guest1"),
            ("team_guest2_tag", "team_guest2"),
            ("team_guest3_tag", "team_guest3"),
        ]:
        ...
        return flask.render_template(
            ...
            std_tag_info=[
                {
                    "tag": s.tag,
                    "original_desc": s.description,
                    "desc_override": settings.get("std_desc_overrides", {}).get(
                        s.tag, s.description
                    ),
                }
                for s in _app.time_standards.header.standards
            ]
            if _app.time_standards
            else [],
    ...
    def route_records_remove(set_id):
        _app.swim_record_sets = [
            s for s in _app.swim_record_sets if s["set_id"] != set_id
        ]
        import base64
        import pickle

        if _app.swim_record_sets:
            _app.settings["swim_record_sets"] = base64.b64encode(
                pickle.dumps(_app.swim_record_sets)
            ).decode("ascii")
        else:
            ...
            return flask.jsonify(
                {
                    ...
                    "relay_url_preprod": _app.settings.get(
                        "azure_relay_url_preprod", ""
                    ),
                    "public_url_preprod": _app.settings.get(
        ...
        try:
            ...
            if "environment" in body:
                ...
                if env not in ("preprod", "prod"):
                    return flask.jsonify(
                        {"error": "environment must be 'preprod' or 'prod'"}
                    ), 400
    ...
    def route_azure_login():
        ...
        body = flask.request.get_json(silent=True) or {}
        tenant_id = (
            body.get("tenant_id") or _app.settings.get("azure_tenant_id") or ""
        ).strip()
        client_id = (
            body.get("client_id") or _app.settings.get("azure_client_id") or ""
        ).strip()
        if not (tenant_id and client_id):
            return flask.jsonify({"error": "tenant_id, client_id are required"}), 400
        audience = (
            body.get("audience") or _app.settings.get("azure_audience") or ""
        ).strip()
        if not audience:
    ...
    def route_azure_login_complete():
        ...
        if ok:
            ...
            try:
                ...
                warning = "failed to persist azure_settings.json"
                logging.exception("save_azure_settings failed after sign-in")
            try:
                _app.azure_relay_client.update_relay_url(_app._active_azure_urls()[0])
                _app.azure_relay_client.start()
            except Exception:
                warning = (
                    warning + "; " if warning else ""
                ) + "failed to start relay worker"
    ...
    def route_azure_rotate_id():
        ...
        try:
            ...
            pass
        return flask.jsonify(
            {"ok": True, "meet_id": new_id, "status": _azure_status_payload()}
        )
    ...
    def route_azure_check_meet_id():
        ...
        if not ok:
            return flask.jsonify(
                {"ok": False, "available": False, "owner": None, "error": err}
            ), 200
    ...
    def route_azure_set_meet_id():
        ...
        try:
            ...
            pass
        return flask.jsonify(
            {"ok": True, "meet_id": result, "status": _azure_status_payload()}
        )
    ...
    def route_azure_qr_png():
        ...
        resp.headers["Content-Disposition"] = 'attachment; filename="scoreboard-qr.png"'
