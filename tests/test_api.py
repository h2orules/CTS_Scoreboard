"""Tests for REST API endpoints (/api/qualifying-info, /api/blank-message)."""
import pytest
...
class TestApiBlankMessage:
    def test_empty_cache_returns_200_empty(self, client):
        resp = client.get('/api/blank-message')
        assert resp.status_code == 200
        assert resp.data == b''

    def test_returns_cached_html(self, client):
        _cache_put('blank_message', '<h1>Hello</h1>')
        resp = client.get('/api/blank-message')
        assert resp.status_code == 200
        assert b'Hello' in resp.data

    def test_etag_header_present(self, client):
        key = _cache_put('blank_message', '<h1>Test</h1>')
        resp = client.get('/api/blank-message')
        assert resp.headers['ETag'] == '"' + key + '"'
    ...
    def test_if_none_match_wrong_returns_200(self, client):
        _cache_put('blank_message', '<h1>Test</h1>')
        resp = client.get('/api/blank-message',
